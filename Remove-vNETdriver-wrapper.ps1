function Remove-vNETdriver-wrapper {
    ...
    .PARAMETER ClusterName
        The cluster you want to connect and retreive the topology information for. Runs against all clusters / hosts if not specified. Doesn't support resourcepools (will only get VMs directly under the cluster).
    .PARAMETER VMname
        The VM you want to connect and retreive the topology information for. Runs against all clusters / hosts if not specified. Doesn't work together with ClusterName parameter.
    .PARAMETER Run
    ...
    foreach ($vm in $vms | Where-Object {
        ...
        $debugStats = Invoke-VMScript -vm $fullVmObject -ScriptText "[System.Environment]::OSVersion.Version.ToString(); (Get-Host).Version.ToString(); Get-ExecutionPolicy" -GuestUser $guestUser -GuestPassword $guestPassword -ScriptType Powershell
        ...
        if ($Run) {
            Copy-VMGuestFile -VM $fullVmObject -LocalToGuest -Source $localScriptToUpload -Destination $guestRemoteFolder -GuestUser $guestUser -GuestPassword $guestPassword
            $fileCopied = $?
            
            if ($?){
                $scriptOutput = Invoke-VMScript -vm $fullVmObject -ScriptText "powershell.exe -ExecutionPolicy Bypass -File $guestRemoteFolder$scriptName" -GuestUser $guestUser -GuestPassword $guestPassword -ScriptType Powershell
